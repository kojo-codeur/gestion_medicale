...
class PDF extends FPDF
{
    function Header()
    {
        ...
        $this->SetXY(10, 25);
        $this->Cell(0, 10, 'Liste d inscription des stagiaires de Mairie de Bujumbura', 0, 1, 'C');
...
// Ajouter une nouvelle colonne "Type" pour indiquer stagiaire ou employé
$header = ['NUM', 'Nom', 'Prenom', 'Adresse', 'Téléphone', 'Nationalité', 'Email', 'Fonction', 'Type'];
$widths = [10, 15, 18, 25, 25, 25, 35, 20, 20]; // Ajuster les largeurs pour inclure la nouvelle colonne
...
foreach ($donnees as $donnee) {
    // Vérification de l'id_catego
    $type = $donnee['id_catego'] === null ? 'Employé' : 'Stagiaire';

    // Ajout des cellules pour chaque colonne
    $pdf->Cell($widths[0], 10, $donnee['id_inscription'], 1, 0, 'L', $fill);
    $pdf->Cell($widths[1], 10, $donnee['nom'], 1, 0, 'L', $fill);
    $pdf->Cell($widths[2], 10, $donnee['prenom'], 1, 0, 'L', $fill);
    $pdf->Cell($widths[3], 10, substr($donnee['adresse'], 0, 30), 1, 0, 'L', $fill);
    ...
    $pdf->Cell($widths[8], 10, $type, 1, 0, 'L', $fill); // Nouvelle colonne "Type"
    $pdf->Ln();
...
// Sortie du PDF
$pdf->Output('I', 'rapport_inscriptions.pdf');
?>
